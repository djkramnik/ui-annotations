...
function nodeVisible(n: Node) {
  let el: Element | null = (n as any).parentElement ?? (n.parentNode as Element | null);
  while (el) { if (!elVisible(el)) return false; el = el.parentElement; }
  return true;
}
