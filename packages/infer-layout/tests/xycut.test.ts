...
describe('bestSplit', () => {
  ...
  it('returns based on size only if centrality score is 0', () => {
    expect(bestSplit({
      ...
      ...makeTestData(
        [[2,3], [21,24]], // largest vertical gutter has bigger gap but less centered
  ...
  function makeTestData(
    ...
    return {
      cleanVGutters: vGutters.map(([x0, x1]) => ([x0 / page.width, x1 / page.width])),
      cleanHGutters: hGutters.map(([y0, y1]) => ([y0 / page.height, y1 / page.height]))
    }
  }
})
